...
for i2=0,n2-1 do begin
  for i1=starty,starty+width do begin
   ...
   rad=sqrt((n1-(i1))*(n1-(i1))+(n2-(i2))*(n2-(i2)))
   ni1=uint(rad*cos(theta1))
   ni2=uint(rad*sin(theta1))
   if (ni1 lt 0) then begin
     ni1=n1*cos(theta1)
   endif
   if (ni2 lt 0) then begin
    ni2=n2*sin(theta1)
   endif
   if (ni1 gt seg1) then begin
    if (ni1 lt seg2) then begin
      w(ni1,ni2,2)=m2max*cos(theta)
      w(ni1,ni2,1)=m2max*sin(theta)
    endif
   endif

   if (ni1 gt seg2) then begin
    if (ni1 lt seg3) then begin
      w(ni1,ni2,2)=cos(theta)*m2max*(ni1-seg2)/(seg3-seg2)
      w(ni1,ni2,1)=sin(theta)*m2max*(ni1-seg2)/(seg3-seg2)

    endif
   endif

   if (ni1 ge seg3) then begin
      w(ni1,ni2,2)=cos(theta)*m2max*(n2-ni1)/(n2-seg3)
      w(ni1,ni2,1)=sin(theta)*m2max*(n2-ni1)/(n2-seg3)
   endif
